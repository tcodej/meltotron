...
app.main = (function() {
    ...
    const
        ...
        templates = {
            menu: {
                open: '<menu x="%1" y="%2" width="%3" height="%4" requireSelection="false" placeholderText="%5" value="1">',
                close: '</menu>'
            },
            option: {
                open: '<option name="%s">',
            ...
            sample: '<sample path="%1" loNote="%2" hiNote="%2" rootNote="%2" tags="%3" />'
        },
        xml = {
            options: [],
            groups: []
                ...
                patches.forEach(function(patch) {
                    ...
                    if (patch.sample) {
                        ...
                        for (i=patch.range.start; i<=patch.range.end; i++) {
                            sample = self.strReplace(templates.sample, [self.getPath(patch.path) + notes[i] +'.wav', midiNote, group.name]);
                            midiNote++;
